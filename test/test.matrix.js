...
describe( 'matrix pdf', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			pdf( matrix( [10,10] ), mat, mu, b );
		}
	});

	it( 'should evaluate the Laplace pdf for each matrix element', function test() {
		var actual;

		actual = matrix( [5,5], 'float64' );
		actual = pdf( actual, mat, mu, b );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( pdf( out, mat, mu, b ).data, expected );
